...
import { createEncounter as apiCreateEncounter, deleteEncounter as apiDeleteEncounter, listEncounters, renameEncounter as apiRenameEncounter } from '../data/encounterDb';
const EncounterContext = createContext(undefined);
export const EncounterProvider = ({ children }) => {
    ...
    const createEncounter = useCallback(async (name) => {
        if (!user) {
    ...
    const renameEncounter = useCallback(async (id, name) => {
        if (!user) {
        ...
        try {
            ...
            setEncounters((current) => current.map((encounter) => (encounter.id === id ? { ...encounter, name: summary.name, updatedAt: summary.updatedAt } : encounter)));
            return summary;
        }
        catch (err) {
    ...
    return _jsx(EncounterContext.Provider, { value: value, children: children });
};
