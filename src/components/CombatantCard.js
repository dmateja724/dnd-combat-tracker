...
const CombatantCard = ({ combatant, isActive, statusPresets, onCenter, onDamage, onHeal, onRemove, onUpdate, onAddStatus, onRemoveStatus }) => {
    ...
    const [selectedStatusId, setSelectedStatusId] = useState(statusPresets[0]?.id ?? 'custom');
            ...
            defeated: isDefeated
                            ...
                            onDamage(amount);
                                    ...
                                    if (amount === 0)
                                        return;
                                    onHeal(amount);
                                }, children: "Apply Heal" })] })] }), _jsxs("div", { className: "card-bottom", children: [_jsxs("section", { className: "status-section", children: [_jsxs("div", { className: "status-head", children: [_jsx("h4", { children: "Status Effects" }), _jsx("button", { type: "button", className: "ghost", onClick: handleOpenStatusPanel, children: "Add" })] }), _jsxs("div", { className: "status-chips", children: [combatant.statuses.length === 0 && _jsx("span", { className: "muted", children: "None" }), combatant.statuses.map((status) => (_jsxs("button", { type: "button", className: "status-chip", style: { backgroundColor: status.color }, onClick: () => onRemoveStatus(status.instanceId), title: status.note || status.description || status.label, children: [_jsx("span", { className: "icon", children: status.icon }), _jsx("span", { children: status.label }), status.remainingRounds !== null ? _jsx("span", { className: "rounds", children: status.remainingRounds }) : null] }, status.instanceId)))] })] }), _jsxs("section", { className: "notes-section", children: [_jsxs("div", { className: "notes-head", children: [_jsx("h4", { children: "Notes" }), _jsx("button", { type: "button", className: "ghost", onClick: () => {
                                            if (isEditingNote) {
                                                setIsEditingNote(false);
                                                setNoteDraft(combatant.note ?? '');
                                            }
