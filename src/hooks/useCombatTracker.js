...
const sortCombatants = (combatants) => [...combatants].sort((a, b) => {
    if (b.initiative === a.initiative) {
        return a.name.localeCompare(b.name);
    }
    return b.initiative - a.initiative;
});
const decrementStatusDurations = (combatants) => combatants.map((combatant) => ({
    ...combatant,
...
const trackerReducer = (state, action) => {
    switch (action.type) {
        ...
        case 'update-combatant': {
            const combatants = sortCombatants(state.combatants.map((combatant) => combatant.id === action.payload.id
                ? {
                    ...combatant,
                    ...
                    hp: {
                        ...
                        current: Math.min(combatant.hp.max, Math.max(0, action.payload.changes.hp?.current ?? combatant.hp.current)),
        ...
        case 'add-status': {
            const combatants = state.combatants.map((combatant) => combatant.id === action.payload.id
                ? {
                    ...combatant,
...
export const useCombatTracker = (encounterId) => {
    const initialEncounterRef = useRef(null);
    ...
    const addCombatant = useCallback((input) => {
        const combatant = {
            id: nanoid(8),
            name: input.name,
            type: input.type,
    ...
    const applyDelta = useCallback((id, delta) => {
        dispatch({ type: 'apply-delta', payload: { id, delta } });
    }, []);
    const setActiveCombatant = useCallback((id) => {
    ...
    const addStatus = useCallback((id, template, remainingRounds, note) => {
        const status = {
            ...template,
    ...
    const removeStatus = useCallback((id, statusId) => {
    ...
    const hydrate = useCallback((encounter) => {
