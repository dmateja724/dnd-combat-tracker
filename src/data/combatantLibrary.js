...
const parseTemplate = (value) => {
    ...
    if (typeof value.id !== 'string' || typeof value.name !== 'string')
        return null;
    if (!isCombatantType(value.type))
        return null;
    if (typeof value.defaultInitiative !== 'number')
        return null;
    if (typeof value.maxHp !== 'number')
        return null;
...
export const listCombatantTemplates = async () => {
    try {
...
export const createCombatantTemplate = async (input) => {
    try {
...
export const updateCombatantTemplate = async (id, input) => {
    try {
...
export const deleteCombatantTemplate = async (id) => {
    try {
        const response = await fetch(`${API_BASE}/${id}`, {
            method: 'DELETE',
            credentials: 'include'
